...
const watchChanges = (dir, lastTimestamp) => {
	timestampForFilesInDirectory(dir).then(timestamp => {
		(!lastTimestamp || (lastTimestamp === timestamp)) ? watchChanges(dir, timestamp) : reload();
	});
};
chrome.management.getSelf(self => {
	if (self.installType === 'development') chrome.runtime.getPackageDirectoryEntry(dir => watchChanges(dir));
});